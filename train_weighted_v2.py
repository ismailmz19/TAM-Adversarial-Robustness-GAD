# -*- coding: utf-8 -*-
# TAM Extension: Angle 5 (Weighted Ensemble) + Adaptive Temperature Selection
...
parser.add_argument('--sep_temp', type=float, default=0.1)
parser.add_argument('--auto_temp', action='store_true', help='Use CV-based adaptive temperature')
...
print(f'  Auto temp  : {args.auto_temp}')
print(f'{"="*60}\n')
...
def reg_edge(emb, adj):
    ...
    return torch.sum(sim_u_u_no_diag * r_inv)
...
def weighted_aggregate(score_list, percentile=0.10, temperature=1.0):
    scores_np = [s.cpu().detach().numpy() if isinstance(s, torch.Tensor) else np.array(s)
                 for s in score_list]
    ...
    weights /= weights.sum()
    stacked = torch.stack(
        [s if isinstance(s, torch.Tensor) else torch.FloatTensor(s) for s in score_list], dim=0)
    weights_t = torch.FloatTensor(weights).unsqueeze(1).to(stacked.device)
    return (stacked * weights_t).sum(dim=0), weights, sep_scores


def compute_auto_temp(sep_scores, base=0.1):
    cv = np.std(sep_scores) / (np.mean(sep_scores) + 1e-8)
    return float(np.clip(base / (cv + 1e-3), 0.05, 2.0))
...
with tqdm(total=args.num_epoch) as pbar:
    ...
    for n_cut in range(args.cutting):
        ...
        msg_w, w_t, sep_t = weighted_aggregate(
            [torch.squeeze(m) for m in message_list], args.sep_percentile, args.sep_temp)
...
# 2. Angle 5 with manual temperature
msg_w5, final_w_k, final_sep_k = weighted_aggregate(
    [torch.squeeze(m) for m in message_mean_list_weighted], args.sep_percentile, args.sep_temp)
score_w5 = 1 - normalize_score(np.array(msg_w5.cpu().detach()))
auc_w5 = roc_auc_score(ano_label, score_w5)
ap_w5 = average_precision_score(ano_label, score_w5, average='macro', pos_label=1)

# 3. Angle 5 with auto temperature
k_sep_scores = np.array([compute_separation_score(
    np.array(torch.squeeze(m).cpu().detach()), args.sep_percentile)
    for m in message_mean_list_weighted])
auto_temp_val = compute_auto_temp(k_sep_scores)
print(f'\n--- Auto Temperature Selection ---')
print(f'  K-level sep scores : {np.round(k_sep_scores, 4)}')
print(f'  CV                 : {np.std(k_sep_scores)/(np.mean(k_sep_scores)+1e-8):.4f}')
print(f'  Auto temp selected : {auto_temp_val:.3f}')

msg_auto, auto_w_k, auto_sep_k = weighted_aggregate(
    [torch.squeeze(m) for m in message_mean_list_weighted], args.sep_percentile, auto_temp_val)
score_auto = 1 - normalize_score(np.array(msg_auto.cpu().detach()))
auc_auto = roc_auc_score(ano_label, score_auto)
ap_auto = average_precision_score(ano_label, score_auto, average='macro', pos_label=1)

print(f'\n{"="*60}')
print(f'  FINAL RESULTS --- {args.dataset}')
print(f'{"="*60}')
print(f'  Original TAM            AUROC: {auc_orig:.4f}  AUPRC: {ap_orig:.4f}')
print(f'  Angle5 manual(t={args.sep_temp:.2f})   AUROC: {auc_w5:.4f}  AUPRC: {ap_w5:.4f}  ({auc_w5-auc_orig:+.4f})')
print(f'  Angle5 auto  (t={auto_temp_val:.2f})   AUROC: {auc_auto:.4f}  AUPRC: {ap_auto:.4f}  ({auc_auto-auc_orig:+.4f})')
print(f'  Manual K-weights : ' + '  '.join([f'k{i+1}={w:.3f}' for i, w in enumerate(final_w_k)]))
print(f'  Auto   K-weights : ' + '  '.join([f'k{i+1}={w:.3f}' for i, w in enumerate(auto_w_k)]))
print(f'  Runtime: {end - start:.1f}s')
print(f'{"="*60}\n')

np.save(f'results_v2_{args.dataset}.npy', {
    'dataset': args.dataset, 'ano_label': ano_label,
    'score_original': score_orig,
    'score_manual': score_w5,
    'score_auto': score_auto,
    'auc_original': auc_orig,   'ap_original': ap_orig,
    'auc_manual': auc_w5,       'ap_manual': ap_w5,
    'auc_auto': auc_auto,       'ap_auto': ap_auto,
    'manual_k_weights': final_w_k,
    'auto_k_weights': auto_w_k,
    'auto_temp': auto_temp_val,
    'k_sep_scores': k_sep_scores,
})
print(f'  Saved results_v2_{args.dataset}.npy')
